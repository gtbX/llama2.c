...
void memory_map_weights(w, p, ptr, shared_weights) 
...
Tokenizer* t; char* tokenizer_path; int vocab_size; {
    ...
    t->max_token_length = bswap_32(t->max_token_length);
    int len;
...
float random_f32(state) // random float32 in [0,1)
unsigned long long *state; {
